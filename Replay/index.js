...
function a(){
	...
	btnLock.addEventListener('click', mouseEvent=>{
		...
		fetch('https://api.github.com/search/repositories?q=topic:AI-Tournaments+topic:Replay+topic:'+replayData.arena).then(response => response.json()).then(response => {
			document.getElementById('default-option').value = (true?'https://ai-tournaments.github.io':'http://127.0.0.1:8887')+'/'+replayData.arena.split('/')[1].replace('Arena','Replay')+'/'; // TODO: Automatically use 127.0.0.1:x when developing.
